...
#define TIMEOUT_SEEK 2
#define SEEK_COUNT   500
      ...
      char *argv[])
{
  ...
  GstElement *pipeline, *source, *wavparse, *convert, *alawencoder, *sink;
  GstBus *bus;
  ...
  pipeline = gst_pipeline_new ("alaw-seek-test");
  source   = gst_element_factory_make ("gnomevfssrc", "gnomevfs-src");
  wavparse = gst_element_factory_make ("wavparse", "wav_parser");
  convert  = gst_element_factory_make ("audioconvert",  "audio_convert");
  alawencoder = gst_element_factory_make ("alawenc", "alaw-encoder");
  ...
  gst_bin_add_many (GST_BIN (pipeline),
                    source, wavparse, convert, alawencoder, sink, NULL);
  ...
  */
  ...
  if(!gst_element_link (convert, alawencoder)){
    g_printerr("Error linking convert to alawencoder\n");
    gst_object_unref (GST_OBJECT (pipeline));
    return -1;
  }

  if(!gst_element_link (alawencoder, sink)){
    g_printerr("Error linking alawencoder to appsink\n");
    gst_object_unref (GST_OBJECT (pipeline));
    return -1;
  }
