/*
  ...
  gst-launch -v videotestsrc ! ffenc_h263p ! rtph263ppay ! udpsink host=127.0.0.1 port=5000 async=false udpsrc port=5000 caps="application/x-rtp,media=video,payload=96,clock-rate=90000,encoding-name=H263-1998" ! rtph263pdepay ! ffdec_h263 ! autovideosink audiotestsrc ! alawenc ! rtppcmapay ! udpsink host=127.0.0.1 port=5001 async=false udpsrc port=5001 caps="application/x-rtp,media=audio,payload=8,clock-rate=8000,encoding-name=PCMA" ! rtppcmadepay ! alawdec ! autoaudiosink
      ...
      char *argv[])
{
    ...
    /* Send audio stream*/ 
    GstElement *audiosrc, *send_convert, *send_resample, *audio_encoder, *rtp_audio_pay, *audio_udpsink;
    /* Receive audio stream */
    GstElement *audio_udpsrc, *rtp_audio_depay, *audio_decoder, *receive_convert, *receive_resample, *audiosink;
    ...
    if (!audiosrc || !send_convert || !send_resample || !audio_encoder || !rtp_audio_pay || !audio_udpsink) {
        g_printerr ("Error creating send audio stream !!! Exiting.\n");
        return -1;
    } 
 
    gst_bin_add_many (GST_BIN (pipeline),
                      audiosrc, send_convert, send_resample, audio_encoder, rtp_audio_pay, audio_udpsink, NULL);

    if (! gst_element_link_many (audiosrc, send_convert, send_resample, audio_encoder, rtp_audio_pay, audio_udpsink, NULL)) {
    ...
    if (!audio_udpsrc || !rtp_audio_depay || !audio_decoder || !receive_convert || !receive_resample || !audiosink) {
        g_printerr ("Error creating receive audio stream !!! Exiting.\n");
        return -1;
    } 
 
    gst_bin_add_many (GST_BIN (pipeline),
                      audio_udpsrc, rtp_audio_depay, audio_decoder, receive_convert, receive_resample, audiosink, NULL);

    if (! gst_element_link_many (audio_udpsrc, rtp_audio_depay, audio_decoder, receive_convert, receive_resample, audiosink, NULL)) {
