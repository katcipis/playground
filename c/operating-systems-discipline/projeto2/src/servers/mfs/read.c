 ...
 *                              fs_getcontig                                 *
 *===========================================================================*/
PUBLIC int fs_getcontig(void)
{
  struct inode *rip;
  int i, j, cont, total,tmp;
  int pcent;
  block_t b, db;
  struct buf *bp, *dbp;
  zone_t zone_indirect;
  int scale;

  printf("fs_getcontig: got inode[%d].\n", fs_m_in.REQ_FD_INODE_NR);
  rip = find_inode(fs_dev, fs_m_in.REQ_FD_INODE_NR);

  if(!rip){
    printf("FS: unavaliable inode by fs_getcontig(), nr: %d\n", fs_m_in.REQ_FD_INODE_NR);
    return EINVAL;
  }
  ...
  i = 1;
  while(rip->i_zone[i] != 0 && i <= 6){
    if(rip->i_zone[i] != 0 ){
      total++;
      if((tmp+1) == rip->i_zone[i])
        cont++;
  ...
  if(rip->i_zone[7] != 0){
    ...
    i = 0;
    zone_indirect = rd_indir(bp, i);		
    while (zone_indirect != 0 && i < 512){			
      total++;
      if((tmp+1) == zone_indirect)
        cont++;
  ...
  /* double indir */
  if(rip->i_zone[8] != 0){
    b = (block_t) rip->i_zone[8] << scale;
    bp = get_block(rip->i_dev, b, (NORMAL) );
			
    i = 0;			
    db = (block_t) rd_indir(bp,i);
			
    while (db != 0 && i < 512) {
      db = db << scale;
      dbp = get_block(rip->i_dev,db,(NORMAL));
      j = 0;
      zone_indirect = rd_indir(dbp,j);
				
      while(zone_indirect != 0 && j < 512) {
        total++;
        if((tmp+1) == zone_indirect)
          cont++;
  ...
  if (total == 0){
    ...
    pcent = (cont*10000)/total;
  }
  
  fs_m_out.REQ_COUNT = pcent;
  printf("fs_getcontig: fs_m_out.REQ_COUNT[%d]\n", fs_m_out.REQ_COUNT);
  return OK;  
}
/*===========================================================================*
 *				fs_readwrite_o				     *
