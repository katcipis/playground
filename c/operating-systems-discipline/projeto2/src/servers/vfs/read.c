 ...
 *                              do_getinode                                  *
 *===========================================================================*/
PUBLIC int do_getinode()
{
  register struct filp *f;
  register struct vnode *vp;
  message m;

  printf("do_getinode: m_in.fd[%d]\n", m_in.fd);
  if ((f=get_filp(m_in.fd)) == NIL_FILP) {
    printf("do_getinode: error checking file descriptor!!! \n", m_in.fd);
    return (err_code);
  }
  ...
  /*Send message with inode*/
  m.m_type = REQ_FCONTIG;
  m.REQ_INODE_NR = vp->v_inode_nr;
  printf("do_getinode: got inode[%d]\n", m.REQ_INODE_NR);
  sendrec(vp->v_fs_e, &m);
  printf("do_getinode: m.REQ_COUNT[%d]\n", m.REQ_COUNT);
