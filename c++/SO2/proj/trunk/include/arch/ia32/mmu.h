...
public:
	...
	IA32_Flags(unsigned int f) : _flags(f) {}
        ...
        /* Removed the hardcoded PRE flag */
	IA32_Flags(Flags f) : _flags(ACC |
				     ((f & Flags::RW)  ? RW  : 0) |
				     ((f & Flags::USR) ? USR : 0) |
				     ((f & Flags::CWT) ? PWT : 0) |
				     ((f & Flags::CD)  ? PCD : 0) |
				     ((f & Flags::CT)  ? CT  : 0) |
                                     ((f & Flags::PRE) ? PRE : 0) | /* Adding the PRE flag here, thanks to Guto for the help :-) */
				     ((f & Flags::IO)  ? PCI : 0) ) {}
    ...
    public:
	    ...
	    : _from(0), _to(pages(bytes)), _pts(page_tables(_to - _from)),
	      _flags(IA32_Flags(flags)), _pt(calloc(_pts)) {
            ...
            db<IA32_MMU>(TRC) << "Chunk(bytes=" << bytes << ", flags=" << flags << ")\n";

	    if(flags & IA32_Flags::CT) {
                db<IA32_MMU>(TRC) << "Chunk(): mapping page contiguous !!!\n";
		_pt->map_contiguous(_from, _to, _flags);
	    } else { 
                db<IA32_MMU>(TRC) << "Chunk(): mapping page not contiguous !!!\n";
	...
	Phy_Addr phy_address() const {
            db<IA32_MMU>(TRC) << "Chunk::phy_address(), _flags = " << _flags << "\n";
    ...
    /* Now we can access it from our Secure_Segment subclass */
    protected:
	unsigned int _from;
	unsigned int _to;
	unsigned int _pts;
