...
ChatPanel::ChatPanel(wxWindow* parent,  buzz::Jid other_jid)
  ...
  SetSizer(sizer);wxT("SVL$");
  
  m_capturePanel = new CapturePanel(this, m_other_jid);
  m_capturePanel->SetBackgroundColour(*wxBLACK);
  m_capturePanel->SetSize(200,200);
  
  m_drawPanel = new DrawPanel(this);
  m_drawPanel->SetBackgroundColour(*wxRED);
  m_drawPanel->SetSize(200,200);
  
  // create
  //m_chat_text = new wxTextCtrl(this, wxID_ANY, wxT(""), wxDefaultPosition, wxDefaultSize,
  ...
  sizer->Add(m_capturePanel, wxGBPosition(1, 3), wxGBSpan(1, 1),
      wxEXPAND | wxRIGHT | wxBOTTOM, 5);
  
  sizer->Add(m_drawPanel, wxGBPosition(1, 4), wxGBSpan(1, 1),
        wxEXPAND | wxRIGHT | wxBOTTOM, 5);
...
void ChatPanel::ReceiveMsg(wxString &text){
  ...
  wxString protocolId = text.Left(4);
  if (protocolId.IsSameAs(wxT("SVL$"))) {
	  wxString position = text.Mid(4);
	  
	  wxStringTokenizer toker(position, wxT("|"));
	  wxString wxX = toker.GetNextToken();
	  wxString wxY = toker.GetNextToken();
	  
	  std::string x_str = std::string(wxConvUTF8.cWC2MB(wxX.wc_str(*wxConvCurrent)));
	  std::string y_str = std::string(wxConvUTF8.cWC2MB(wxY.wc_str(*wxConvCurrent)));
	  
	  unsigned int x = atoi(x_str.c_str());
	  unsigned int y = atoi(y_str.c_str());
	  
	  m_drawPanel->MoveThatThing(x, y);
  } else {
